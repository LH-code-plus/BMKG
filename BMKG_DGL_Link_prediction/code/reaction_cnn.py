...
def create_char_dict(df):
    ...
    """
    chars = set()
    for idx, smiles in enumerate(df['reaction_eq_annotation']):
        try:
...
# 读取CSV文件
df = pd.read_csv("../data/csv/Reaction_Smiles.csv")

# 创建字符字典
char_dict = create_char_dict(df)

# 将SMILES字符串转换为数字序列
sequences = [smiles_to_sequence(smiles, char_dict) for smiles in df['reaction_eq_annotation']]
# 将序列转换为numpy数组
...
# 确定要保留的列
columns_to_keep = ['id', 'reaction_eq_annotation', 'vector']
...
# 保存结果
df_result.to_csv("../data/csv/Reactions_cnn.csv", index=False)
