...
def convert_gene_data_to_cypher(data, output_file):
  cypher_statements = []
  for row in data:
    # Create node
    node_id = row['id']
    create_statement = f'CREATE (n:Gene {{id: "{node_id}"}}) RETURN n ;'
    cypher_statements.append(create_statement)

    # Set node properties
    set_properties = f'MATCH (n:Gene {{id: "{node_id}"}}) SET '
    properties = []
...
# Example usage
data_file = '../datas/genes.csv'  # Replace with your CSV file path
output_file = 'cypher_genes.txt'  # Replace with your desired output file path
...
convert_gene_data_to_cypher(data, output_file)
