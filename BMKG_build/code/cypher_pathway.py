...
def convert_protein_data_to_cypher(data, output_file):
  ...
  for row in data:
    # Create node
    pathway = row['KEGG_pathway']
    # Handle lists and strings with potential quotes
    value_pathway = str(pathway)
    items_UEC = value_pathway.split(",")
    ...
    if len(trimmed_items_UEC_list) == 0:
        ...
        for item in trimmed_items_UEC_list:
            #value_str = repr(item)
            create_statement = f'CREATE (n:Pathway {{name: "{item}"}}) RETURN n ;'
...
output_file = 'cypher_pathway.txt'  # Replace with your desired output file path
